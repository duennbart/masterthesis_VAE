...
def calculate_fid_given_array(pre_calc_path,x, inception_path, low_profile=False):
    ''' Calculates the FID of two paths. '''
    inception_path = check_or_download_inception(inception_path)
    if not os.path.exists(pre_calc_path):
        raise RuntimeError("Invalid path: %s" % pre_calc_path)
    graph2 = tf.Graph()
    with graph2.as_default() as g:
        ...
        with tf.Session(graph=graph2) as sess:
            sess.run(tf.global_variables_initializer())
            m1, s1 = _handle_path(pre_calc_path, sess, low_profile=low_profile)
            m2, s2 = calculate_activation_statistics(x, sess)
            fid_value = calculate_frechet_distance(m1, s1, m2, s2)
            return fid_value
    tf.keras.backend.clear_session()

def save_mu_sigma_given_paths(path,save_path, inception_path, low_profile=False):
    ''' Calculates the FID of two paths. '''
    inception_path = check_or_download_inception(inception_path)
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        mu, sigma = _handle_path(path, sess, low_profile=low_profile)
